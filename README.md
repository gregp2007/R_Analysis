...
To ensure the variable names were properly descriptive the prefix "Avg" was appended to the begining of each descriptive variable to avoid any confusion when viewing the data. The variables and data descriptions will be described in more detail in the CodeBook.
...
Measurements were recorded for 561 different variables, for the purpose of this summarized dataset we were asked to only use the mean and std variables. I identified 79 variables that met this critera, I determined that the mean for the angle features did not fall into the Time and Frequency measurement variables so these were excluded from our summarized data set.
...
subject_train.txt- The subjects that particpated in the train group for each observation (7352 observatuions, 1 variables)
x_train.txt- All measurment observations  for activties performed by subjects in the train group (7352 observations, 561 variables)
y_train.txt- Activities for each observation that subjects permormed in the train group (7352 observations, 1 variable)

subject_test.txt- The subjects that particpated in the test group for each observation (2947 observatuions, 1 variables)
x_test.txt- All measurment observations for activties performed by subjects in test group (2947 observations, 561 variables)
y_test.txt- Activities for each observation that subjects permormed in the test group (2947 observations, 1 variable)

activity_labels.txt - the activty descriptors that would be associated to the y files
...
Read in the festures.txt file and save to features.Validate the names and review for uniqueness.
...
save as subjects. Validate for exxpected variables and observations.
...
Since we have already combined all data and the last step is to append the activty labels to this dataset we will be ok
...
We will now write a dplyr chain which groups by Activity Code, Activity Desc, Subject abd Subject Group. 
We then will chain this with the summarize function and will performa a mean function on each variable. 
